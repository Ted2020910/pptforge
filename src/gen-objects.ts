...
import { latexToOMML } from './formula-converter'
...
export function addMediaDefinition(target: PresSlide, opt: MediaProps): void {
	...
	strExtn = opt.extn || (strData ? strData.split(';')[0].split('/')[1] : (strPath.split('.').pop() || '').split('?')[0]) || 'mp3'
	...
	if (opt.autoPlay) slideData.options.autoPlay = opt.autoPlay
	if (opt.loop) slideData.options.loop = opt.loop
...
export function addShapeDefinition(target: PresSlide, shapeName: SHAPE_NAME, opts: ShapeProps): void {
	...
	// 4: Handle texture fill - add image to media relations
	if (options.fill && typeof options.fill === 'object' && options.fill.type === 'texture' && options.fill.texture) {
		const texture = options.fill.texture
		const strImagePath = texture.path || ''
		const strImageData = texture.data || ''

		// Validate texture input
		if (!strImagePath && !strImageData) {
			console.error('ERROR: Texture fill requires either \'data\' or \'path\' parameter!')
		} else {
			...
			} else if (strImageData && /image\/(\w+);/.exec(strImageData)) {
				strImgExtn = /image\/(\w+);/.exec(strImageData)[1]
			}

			// Get new relation ID
			const textureRid = getNewRelId(target)

			// Check for duplicate media to reuse existing Target
			const dupeItem = target._relsMedia.filter(
				item => item.path && item.path === strImagePath && item.type === 'image/' + strImgExtn && !item.isDuplicate
			)[0]

			// Add texture image to media relations
			target._relsMedia.push({
				path: strImagePath || 'preencoded.' + strImgExtn,
				type: 'image/' + strImgExtn,
				extn: strImgExtn,
			...
			// Store the rId in options for later use in XML generation
			if (!(options.fill as any)._textureRid) {
				(options.fill as any)._textureRid = textureRid
			}
		}
	}
	...
	arrRows.forEach(row => {
		row.forEach((cell, idy) => {
			...
			*/
			...
			// C: 公式自动检测：如果表格层面开启了 autoDetectFormulas，则默认让单元格也继承该配置
			if ((opt as any).autoDetectFormulas && (!row[idy].options || typeof (row[idy].options as any).autoDetectFormulas === 'undefined')) {
				(row[idy].options as any).autoDetectFormulas = true
			}
...
export function addTextDefinition(target: PresSlide, text: TextProps[], opts: TextPropsOptions, isPlaceholder: boolean): void {
	// STEP 0: Handle automatic LaTeX formula detection if enabled
	if (opts?.autoDetectFormulas && text && text.length > 0) {
		// Process each text segment to detect and embed formulas inline
		text.forEach(textItem => {
			if (typeof textItem.text === 'string' && hasFormula(textItem.text)) {
				const segments = parseTextWithFormulas(textItem.text).map(segment => {
					if (segment.type === 'text') return { type: 'text', text: segment.content }
					return { type: 'formula', latex: segment.content, isBlock: segment.isBlock }
				})
				// Replace original text with mixed segments (handled later in XML generation)
				// Disable further auto-detection for this item to avoid recursion
				textItem.text = segments as any
			}
		})
	}
...
/**
 * Adds a formula (LaTeX math equation) to a slide
 * @param {PresSlide} target - slide object that the formula will be added to
 * @param {FormulaProps} options - formula options including LaTeX string and positioning
 */
export function addFormulaDefinition(target: PresSlide, options: FormulaProps): void {
	// 自动转换 LaTeX 到 OMML（如果用户没有提供 _omml）
	// Automatically convert LaTeX to OMML (if user didn't provide _omml)
	let omml = options._omml || ''
	if (!omml && options.latex) {
		try {
			omml = latexToOMML(options.latex)
		} catch (error) {
			console.error(`Failed to convert LaTeX to OMML: ${(error as Error).message}`)
			// 如果转换失败，使用空字符串
			omml = ''
		}
	}
	...
	const newObject: ISlideObject = {
		...
		options: {
			...
			h: options.h || 0.5,
			latex: options.latex,
			fontSize: options.fontSize || DEF_FONT_SIZE,
			color: options.color || DEF_FONT_COLOR,
			...
			objectName: options.objectName
				? encodeXmlEntities(options.objectName)
				: `Formula ${target._slideObjects.filter(obj => obj._type === SLIDE_OBJECT_TYPES.formula).length + 1}`,
		},
	}
...
/**
 * Adds an icon to a slide by fetching it from an icon library
 * @param {PresSlide} target - slide object that the icon will be added to
 * @param {IconProps} options - icon options including icon name, library, and positioning
 * @returns {Promise<void>} Promise that resolves when the icon is added
 */
export async function addIconDefinition(target: PresSlide, options: IconProps): Promise<void> {
	// Dynamic import of icon-helper using runtime path resolution
	// This avoids Rollup trying to resolve the module at build time
	let iconHelper: any
	const paths = [
		'../../../utils/icon-helper.mjs',
		'../../utils/icon-helper.mjs',
		'../../../../utils/icon-helper.mjs',
	]
	...
	for (const path of paths) {
		try {
			// Use Function constructor to create dynamic import that Rollup won't try to resolve
			const importPath = path
			// @ts-ignore - Dynamic import with variable path
			iconHelper = await new Function('path', 'return import(path)')(importPath)
			break
		} catch (error) {
	...
	if (!iconHelper) {
		console.error('ERROR: icon-helper.mjs not found. Please ensure utils/icon-helper.mjs exists.')
		console.error(`Tried paths: ${paths.join(', ')}`)
		throw new Error(`Icon helper module not found: ${lastError?.message || 'All import paths failed'}`)
	}

	// Get icon name and library
	const iconName = options.icon
	const library = options.library || 'heroicons'
	const style = options.style || (library === 'material' || library === 'material-icons' ? 'outlined' : 'solid')
	const color = options.color || '#000000'

	// Fetch icon SVG and convert to base64
	let iconBase64: string
	try {
		iconBase64 = await iconHelper.getIcon(iconName, {
			library: library,
			style: style,
			color: color,
		})
	} catch (error) {
		console.error(`Failed to fetch icon "${iconName}" from library "${library}": ${(error as Error).message}`)
		throw error
	}

	// Create ImageProps from IconProps
	const imageOptions: ImageProps = {
		...
		objectName: options.objectName || `Icon ${target._slideObjects.filter(obj => obj._type === SLIDE_OBJECT_TYPES.image).length + 1}`,
	}

	// Use existing addImageDefinition to add the icon as an image
	addImageDefinition(target, imageOptions)
}
