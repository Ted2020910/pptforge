...
function parseTextToLines(cell: TableCell, colWidth: number, verbose?: boolean): TableCell[][] {
	...
	*/
	...
	if (cell.text && cell.text.toString().trim().length === 0) {
		...
		inputCells.push({ _type: SLIDE_OBJECT_TYPES.tablecell, text: ' ', options: cell.options })
	} else if (typeof cell.text === 'number' || typeof cell.text === 'string') {
		inputCells.push({ _type: SLIDE_OBJECT_TYPES.tablecell, text: (cell.text || '').toString().trim(), options: cell.options })
	...
	tableRows.forEach((row, iRow) => {
		...
		row.forEach((cell, iCell) => {
			const newCell: TableCell = {
				...
				text: [],
				// 深拷贝 options 以避免修改原始对象（支持多个单元格共享同一 options）
				options: cell.options ? { ...cell.options } : {},
			}
		...
		while (!isDone) {
			...
			if (emuTabCurrH + emuLineMaxH > emuSlideTabH) {
				...
				currTableRow = []
				row.forEach(cell => currTableRow.push({ _type: SLIDE_OBJECT_TYPES.tablecell, text: [], options: cell.options ? { ...cell.options } : {} }))
