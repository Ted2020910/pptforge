...
import { makeXmlTiming, makeXmlTransition } from './gen-animation'
import {
	...
	slide._slideObjects.forEach((slideItemObj: ISlideObject, idx: number) => {
		...
		switch (slideItemObj._type) {
			case SLIDE_OBJECT_TYPES.table:
				...
				slideItemObj._animShapeId = intTableNum * slide._slideNum + 1
				strXml +=
			...
			case SLIDE_OBJECT_TYPES.placeholder:
				...
				slideItemObj._animShapeId = idx + 2
				// <Hyperlink>
			...
			case SLIDE_OBJECT_TYPES.media:
				if (slideItemObj.mtype === 'online') {
					...
					slideItemObj._animShapeId = slideItemObj.mediaRid + 2
...
function slideObjectRelationsToXml (slide: PresSlide | SlideLayout, defaultRels: Array<{ target: string, type: string }>): string {
	let currentRid = 0 // current rId counter
	...
	// STEP 1: Add default rels FIRST (slideLayout must be rId1)
	defaultRels.forEach((rel) => {
		currentRid++
		strXml += `<Relationship Id="rId${currentRid}" Type="${rel.type}" Target="${rel.target}"/>`
	})

	// STEP 2: Add all rels for this Slide
	slide._rels.forEach((rel: ISlideRel) => {
		currentRid++
		if (rel.type.toLowerCase().includes('hyperlink')) {
			if (rel.data === 'slide') {
				strXml += `<Relationship Id="rId${currentRid}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide" Target="slide${rel.Target}.xml"/>`
			} else {
				strXml += `<Relationship Id="rId${currentRid}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="${rel.Target}" TargetMode="External"/>`
			}
		} else if (rel.type.toLowerCase().includes('notesSlide')) {
			strXml += `<Relationship Id="rId${currentRid}" Target="${rel.Target}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/notesSlide"/>`
		}
	})
	; (slide._relsChart || []).forEach((rel: ISlideRelChart) => {
		currentRid++
		strXml += `<Relationship Id="rId${currentRid}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart" Target="${rel.Target}"/>`
	})
	; (slide._relsMedia || []).forEach((rel: ISlideRelMedia) => {
		currentRid++
		if (rel.type.toLowerCase().includes('image')) {
			strXml += `<Relationship Id="rId${currentRid}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="${rel.Target}"/>`
		} else if (rel.type.toLowerCase().includes('audio')) {
			// As media has *TWO* rel entries per item, check for first one, if found add second rel with alt style
			if (strXml.includes(' Target="' + rel.Target + '"')) {
				strXml += `<Relationship Id="rId${currentRid}" Type="http://schemas.microsoft.com/office/2007/relationships/media" Target="${rel.Target}"/>`
			} else {
				strXml += `<Relationship Id="rId${currentRid}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/audio" Target="${rel.Target}"/>`
			}
			...
			if (strXml.includes(' Target="' + rel.Target + '"')) {
				...
				strXml += `<Relationship Id="rId${currentRid}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/video" Target="${rel.Target}"/>`
			}
		} else if (rel.type.toLowerCase().includes('online')) {
			// As media has *TWO* rel entries per item, check for first one, if found add second rel with alt style
			if (strXml.includes(' Target="' + rel.Target + '"')) {
				strXml += `<Relationship Id="rId${currentRid}" Type="http://schemas.microsoft.com/office/2007/relationships/image" Target="${rel.Target}"/>`
			} else {
				strXml += `<Relationship Id="rId${currentRid}" Target="${rel.Target}" TargetMode="External" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/video"/>`
			}
		}
	})
...
export function makeXmlSlide (slide: PresSlide): string {
	return (
		...
		`${makeXmlTransition(slide._transition)}` +
		`${makeXmlTiming(slide)}` +
		'</p:sld>'
	)
}
