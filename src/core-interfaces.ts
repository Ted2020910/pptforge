...
export interface GradientStop {
	/**
	 * Position of gradient stop (0-100)
	 * @example 0 // start position
	 * @example 50 // middle position
	 * @example 100 // end position
	 */
	position: number
	/**
	...
	color: Color
	/**
	 * Transparency at this gradient stop (0-100)
	 * @default 0
	 */
	transparency?: number
}
...
export interface GradientFillProps {
	...
	stops: GradientStop[]
	/**
	 * Angle for linear gradient (degrees, 0-359)
	 * @default 0
	 */
	angle?: number
	/**
...
// Texture/Image Fill
export interface TextureFillProps {
	/**
	 * Image path or data
	 */
	path?: string
	...
	transparency?: number
	/**
	 * Horizontal alignment for non-tiled images
	 * @default 'center'
	 */
	alignX?: 'left' | 'center' | 'right'
	/**
	 * Vertical alignment for non-tiled images
	 * @default 'middle'
	 */
...
export interface ReflectionProps {
	...
	blur?: number
	/**
	 * Distance from shape (points, 0-100)
	 * @default 0
	 */
	distance?: number
	/**
	...
	transparency?: number
	/**
	 * Size relative to shape (percent, 0-100)
	 * @default 100
	 */
	size?: number
}
...
export interface Bevel3DProps {
	/**
	 * Bevel type
	 */
	type: 'relaxedInset' | 'circle' | 'slope' | 'cross' | 'angle' | 'softRound' | 'convex' | 'coolSlant' | 'divot' | 'riblet' | 'hardEdge' | 'artDeco'
	/**
	 * Width (points, 0-1584)
	 * @default 6
	 */
...
export interface Shape3DProps {
	 ...
	 * Top bevel
	 */
	topBevel?: Bevel3DProps
	/**
	 * Bottom bevel
	 */
	bottomBevel?: Bevel3DProps
	/**
	 * Extrusion height (points)
	 */
	 ...
	 * Lighting type
	 */
	lighting?: 'legacyFlat1' | 'legacyFlat2' | 'legacyFlat3' | 'legacyFlat4' | 'legacyNormal1' | 'legacyNormal2' | 'legacyNormal3' | 'legacyHarsh1' | 'legacyHarsh2' | 'legacyHarsh3' | 'legacyHarsh4' | 'threePt' | 'balanced' | 'soft' | 'harsh' | 'flood' | 'contrasting' | 'morning' | 'sunrise' | 'sunset' | 'chilly' | 'freezing' | 'flat' | 'twoPt' | 'glow' | 'brightRoom'
	/**
	 * Lighting angle (degrees, 0-359)
	 */
...
export interface ShapeFillProps {
	 ...
	 * @default 'solid'
	 */
	type?: 'none' | 'solid' | 'gradient' | 'pattern' | 'texture'

	/**
	 * Gradient fill properties (when type='gradient')
	 */
	gradient?: GradientFillProps

	/**
	 * Pattern fill properties (when type='pattern')
	 */
	pattern?: PatternFillProps

	/**
	 * Texture/Image fill properties (when type='texture')
	 */
	texture?: TextureFillProps

	/**
	 * Internal: Texture image relationship ID
	 * @internal
	 */
	_textureRid?: number
...
/**
 * Animation type - supported animation effects
 * P0 (core): appear, fadeIn, flyIn, wipe, zoom, disappear, fadeOut, flyOut
 * P1 (filter extensions): blinds, box, checkerboard, circle, diamond, dissolve, plus, randomBars, split, strips, wedge, wheel, random
 */
export type AnimationType =
	// P0 core entrance
	| 'appear' | 'fadeIn' | 'flyIn' | 'wipe' | 'zoom'
	// P0 core exit
	| 'disappear' | 'fadeOut' | 'flyOut'
	// P1 filter extensions
	| 'blinds' | 'box' | 'checkerboard' | 'circle' | 'diamond'
	| 'dissolve' | 'plus' | 'randomBars' | 'split' | 'strips'
	| 'wedge' | 'wheel' | 'random'

/**
 * Animation configuration - attached to element options
 * @example { type: 'fadeIn', dur: 500, trigger: 'onClick' }
 * @example { type: 'flyIn', dir: 'bottom', dur: 500, trigger: 'afterPrev', delay: 200 }
 * @example [{ type: 'flyIn', dir: 'bottom' }, { type: 'fadeOut', trigger: 'onClick' }]
 */
export interface AnimationProps {
	...
	type: AnimationType
	/**
	 * Animation class: entrance, exit, or emphasis
	 * @default 'entrance'
	 */
	class?: 'entrance' | 'exit'
	/**
	 * Direction for directional animations (flyIn, wipe, etc.)
	 * @example 'bottom'
	 */
	dir?: 'top' | 'bottom' | 'left' | 'right'
	/**
	 * In/Out for shape-based animations (box, diamond, circle)
	 * @default 'in'
	 */
	inOut?: 'in' | 'out'
	/**
	 ...
	 * - onClick: requires a mouse click (default)
	...
	/**
	 * Order of this animation within the slide (1-based)
	 * - If not specified, order follows the addXxx() call sequence
	 */
	order?: number
	/**
	 * For text elements: animate each paragraph separately
	 * @default false
	 */
	byParagraph?: boolean
}
 ...
 * @example { type: 'fade', dur: 700 }
 * @example { type: 'wipe', dir: 'left', speed: 'slow' }
 */
export interface TransitionProps {
	...
	dur?: number
	/**
	 * Speed preset (alternative to dur)
	 */
	speed?: 'slow' | 'med' | 'fast'
	/**
...
export interface MediaProps extends PositionProps, DataOrPathProps, ObjectNameProps {
	...
	path?: string
	/**
	 * Auto-play video when slideshow starts
	 * @default false
	 */
	autoPlay?: boolean
	/**
...
// text ===========================================================================================
/**
 * @deprecated v4.0.0 - use `GlowProps` instead
 */
export type TextGlowProps = GlowProps
...
export interface TextPropsOptions extends PositionProps, DataOrPathProps, TextBaseProps, ObjectNameProps {
	...
	wrap?: boolean
	/**
	 * Automatically detect and render LaTeX formulas in text
	 * - Detects inline formulas: $...$
	 * - Detects block formulas: $$...$$
	 * @since v3.13.0
	 * @default false
	 * @example slide.addText('The formula is $E=mc^2$ in physics', { autoDetectFormulas: true })
	 */
	autoDetectFormulas?: boolean
...
export interface FormulaProps extends PositionProps, ObjectNameProps {
	...
	/**
	 * Internal: OMML XML string (generated from LaTeX)
	 * @internal
	 */
	_omml?: string
	/**
...
/**
 * Add icon to Slide
 * @example slide.addIcon({ icon: 'star', library: 'heroicons', color: '#FFC000', x: 1, y: 1, w: 2, h: 2 })
 * @example slide.addIcon({ icon: 'check-circle', library: 'fontawesome', color: '#00B050', x: 1, y: 1 })
 */
export interface IconProps extends PositionProps, ObjectNameProps {
	/**
	 * Icon name
	 * @example 'star', 'check-circle', 'heart'
	 */
	icon: string
	/**
	 * Icon library
	 * - 'heroicons' | 'fontawesome' | 'material' | 'lucide' | 'bootstrap'
	 * @default 'heroicons'
	 */
	library?: 'heroicons' | 'fontawesome' | 'material' | 'lucide' | 'bootstrap' | 'fa' | 'hero' | 'material-icons' | 'bootstrap-icons'
	/**
	 * Icon style (depends on library)
	 * - Font Awesome: 'solid' | 'regular' | 'brands'
	 * - Heroicons: 'solid' | 'outline'
	 * - Material Icons: 'outlined' | 'filled' | 'rounded' | 'sharp'
	 * @default 'solid' (for heroicons/fontawesome) or 'outlined' (for material)
	 */
	style?: string
	/**
	...
	color?: string
	/**
	 * Image sizing options (same as ImageProps)
	 */
	sizing?: {
		type: 'contain' | 'cover' | 'crop'
		w: Coord
...
export interface ISlideObject {
	...
	// animation shape id (populated during XML generation)
	_animShapeId?: number
	// text
	text?: TextProps[]
	// table
