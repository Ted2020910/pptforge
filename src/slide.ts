...
export default class Slide {
	 ...
	 * @example slide.transition = { type: 'fade', dur: 700 }
	 * @example slide.transition = { type: 'wipe', dir: 'left', speed: 'slow' }
	 */
	...
	/**
	 * Add formula (LaTeX math equation) to Slide
	 * @param {FormulaProps} options - formula options
	 * @return {Slide} this Slide
	 * @example slide.addFormula({ latex: 'E = mc^2', x: 1, y: 2, w: 4, h: 0.5 })
	 * @example slide.addFormula({ latex: '\\frac{a}{b}', x: 1, y: 3 })
	 * @example slide.addFormula({ latex: '\\int_0^\\infty e^{-x^2} dx', x: 1, y: 4, fontSize: 16 })
	 */
	...
	/**
	 * Add icon to Slide (from icon library)
	 * @param {IconProps} options - icon options
	 * @return {Promise<Slide>} Promise that resolves to this Slide
	 * @example await slide.addIcon({ icon: 'star', library: 'heroicons', color: '#FFC000', x: 1, y: 1, w: 2, h: 2 })
	 * @example await slide.addIcon({ icon: 'check-circle', library: 'fontawesome', color: '#00B050', x: 1, y: 1 })
	 * @example await slide.addIcon({ icon: 'heart', library: 'heroicons', color: '#FF6B6B', x: 1, y: 1, w: 1.5, h: 1.5 })
	 */
	async addIcon(options: IconProps): Promise<Slide> {
		await genObj.addIconDefinition(this, options)
		return this
	}
}
